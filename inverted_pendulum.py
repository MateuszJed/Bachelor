...
import numpy as np,cv2,math,time,matplotlib.pyplot as plt,sys,keyboard
...
def find_lqr_control_input(cart,pendulum,time_delta,error,previous_error,theta_dot,x_dot,g):
	...
	A = np.matrix(  [
					[0,1,0,0],
					[0,b*d/cart.mass,b*pendulum.ball_mass/cart.mass,0],
					[0,0,0,1],
					[0,-b*d/(cart.mass*pendulum.length),-b*(cart.mass+pendulum.ball_mass)*g/(cart.mass*pendulum.length),0]
					])
...
def main(control_technique):
	...
	pendulum = Pendulum(1.3,0,mass_of_ball)
