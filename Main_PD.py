...
def main():
    ...
    while 1:
        ...
        if x_send > -50 and reference_point_x == 0:
        ...
        if reference_point_x != 0 and reference_point_y !=0:
            ...
            if state.runtime_state > 1 and detected:
                ...
                try: 
                    ...
                    print(info)
                #End regulation if error in x and y is small 
                # Controll = False if (abs(error_x) < 0.027 and abs(error_y) < 0.21) else True
            ...
            if keyboard.is_pressed("esc") or running:  # Break loop with ESC-key
                info_csv_1 = [f"Constrain_x: {Constrain_x}, Constrain_y: {Constrain_y}, "]
                info_csv_2 = [f"Kp_x:{Kp_x}, Kp_y:{Kp_y}, Kd_x:{Kd_x}, Kd_y:{Kd_y}"]
                header = ["Time","X","Y"]
                with open(r'C:\Users\mateusz.jedynak\OneDrive - NTNU\Programmering\Python\Prosjekt\Bachelor\Source\Bachelor\Data\x_pix_pid.csv', 'w',newline="") as f:
