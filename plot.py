...
for n, file in enumerate(files):
    ...
    plot_graps(t, x, 1,"PID-controller in {}-direction ".format(format_x[0]), "Time[s]", "Angle between end effector and payload [radian]", f"Trail: {n+1}")
# x = np.ones(11)*0.043303780712741435
# plt.plot(x, color = 'blue', linewidth=1, linestyle='-.')
for n, file in enumerate(files):
    t, x, y = read_csv_data_to_list(file)
    plot_graps(t, y, 2,"PID-controller in {}-direction ".format(format_y[0]), "Time[s]", "Angle between end effector and payload [radian]", f"Trail: {n+1}")
