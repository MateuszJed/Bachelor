    ...
    while 1:
        ...
        if reference_point_x != 0 or reference_point_y !=0:
            ...
            if abs(angle[0]) < 0.04 and abs(angle[1]) < 0.04 and end_time_pos > 2:
                ...
                # p_out_x = reference_point_x + 0.2*error_x + 0.005*dedt + 0.005*eintegral_x
                ...
                # p_out_y = reference_point_y + 0.2*error_y + 0.005*dedt + 0.005*eintegral_y
                if start_time_traj != 0:
                    t = time.time() - start_time_traj
                # print(t)
                end_time_pos = 15
                parameters_to_trajectory_x = inital_parameters_traj(start_pose[0],0,0,0,     0,      11,    5)
                parameters_to_trajectory_y = inital_parameters_traj(start_pose[1],-0.885,0,0,     0,      11,    5)
                                # Trajectory for y
            ...
            if keyboard.is_pressed("esc") or t > 10:  # Break loop with ESC-key
                state = con.receive()
                # ====================mode 3===================
